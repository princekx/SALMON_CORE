...
class RetrieveMogrepsColdSurgeData(Task):
    """
...
class ComputeMogrepsColdSurgeIndices(Task):
    """
    ...
    def _member_directories(self):
        """Return expected 3-digit member directories based on model."""
    ...
    def _forecast_steps(self):
        """Return forecast-step strings based on model."""
        model = self.config_values["model"]
        if model == "gpm":
            return ["000"]
        else:
    ...
    def process_forecast_data(self, date, members):
        ...
        For models like GPM that may not have wind data, synthetic zero-valued
        wind fields are created for compatibility with visualization pipelines.
        """
        fc_times = self._forecast_steps()
        raw_root = self.config_values["raw_dir"]
        out_root = self.config_values["cs_processed_dir"]
        regrid_to_obs = bool(self.config.get("regrid_to_obs", False))
        model = self.config_values["model"]

        logger.info("Computing Cold Surge indices for %s on %s", model, date.strftime("%Y-%m-%d"))
        ...
        for varname, spec in VAR_SPECS.items():
            ...
            for mem in members:
                ...
                # For .nc files (GPM), also check for NetCDF format
                if not any(os.path.exists(f) for f in files):
                    nc_files = [f.replace('.pp', '.nc') for f in files]
                    files = nc_files
                ...
                try:
                    if varname == "precip":
                        ...
                        try:
                            ...
                            logger.warning("Wind data not found for member %s (%s). Creating synthetic zeros.", mem, e)
                            ...
                            try:
                                ...
                                cube.rename(spec["iris_var"])
                            except:
                                logger.error("Could not create synthetic wind data for member %s", mem)
                                continue
                    ...
                    logger.error("Error processing member %s: %s", mem, e)
                    continue
...
class DisplayMogrepsColdSurgeMaps(Task):
    """
